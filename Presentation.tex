...
\begin{frame}{Universal Coefficient Theorem}{Is \(\ZZ\) enough to recover homology through \(A\)?}
  ...
  \[
    ...
    H_n(\G;A)
    \xrightarrow{\highlight[blue]{\pi_n^{\G}}}
...
\begin{frame}{Mayer-Vietoris vs. Moore-Mayer-Vietoris}{From saturated covers to homology.}
  \begin{columns}[t]
    \begin{column}{0.483\hsize}
      ...
      \(U_1,\ U_2,\ U_1\cap U_2\) contractible.\\[0.4em]
...
\[
\G|_U \coloneqq \{g\in \G \mid s(g)\in U,\ r(g)\in U\},
\qquad
(\G|_U)_0 = U,
\]
...
\(\G\) ample \'etale, \(A\) a topological abelian group,
...
\begin{frame}{Example: Diaconu-Renault Groupoid}{Computing Homology with Moore--Mayer--Vietoris + UCT.}
  ...
  \(E_A\) a finite directed graph whose adjacency matrix is \(A\).\\[0.2cm]
  The $\highlight[red]{\text{infinite path space}}$ is given by $\highlight[blue]{\text{Sims 2021, 2.5:}}$\\
  \(E_A^\infty
  = \left\{(e_n)_{n\ge 1}\in E_A^{\mathbb N}\ \middle|\ r(e_n)=s(e_{n+1})\ \text{for all } n\ge 1 \right\}
  \) with \\
  \(r(x,n,y)=x\), \(s(x,n,y)=y\), \(1_x=(x,0,x)\),\\
  \((x,n,y)^{-1}=(y,-n,x)\), \((x,n,y) \cdot (y,m,z)=(x,n+m,z)\) if \(s(x,n,y)=r(y,m,z)\).\\[0.2cm]
