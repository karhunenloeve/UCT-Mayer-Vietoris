...
\begin{frame}{Ample groupoids}{What do we investigate?}
  ...
  \[
    u(x)=1_x,
    \qquad\highlight[red]{s}(g)=g^{-1}\cdot g,
    \qquad
...
\begin{frame}{$\boldsymbol{C_c(\G,\ZZ)}$}{The group is generated by $\chi_U$ for $U \in \operatorname{Bis}(\G)$.}
    \(\highlight[blue]{\text{Lemma:}}\)  Let $\G$ be ample, then every $f \in C_c(\G,\ZZ)$ is locally constant and has compact open support for discrete $\ZZ$. $C_c(\G,\ZZ)$ is generated as abelian group by $\chi_U$ with $U \in \operatorname{Bis}(\G)$.
    \vspace{0.2cm}
...
\begin{frame}{$\boldsymbol{C_c(\G,\ZZ)}$}{The group is generated by $\chi_U$ for $U \in \operatorname{Bis}(\G)$.}
    ...
    Proof:
    \begin{itemize}
        \item \(\highlight[blue]{\text{Refine the cover:}}\) For $1 \leq k,r \leq \ell_j$ define $W_{j,k} \coloneqq U_{j,k} \setminus\bigcup_{r<k} U_{j,r}$. Each $W_{j,r}$ is compact open, as difference of compact open sets. Moreover $W_{j,k} \subset U_{j,k}$, hence $W_{j,k}$ is a bisection. The sets $W_{j,1}, \ldots, W_{j,\ell_j}$ are pairwise disjoint and satisfy $S_j = \bigsqcup_{k=1}^{\ell_j} W_{j,k}$, so $\chi_{S_j}(g) = \sum_{k=1}^{\ell_j} \chi_{W_{j,k}}(g)$. Therefore
        \begin{align}
            f(g) = \sum_{j=1}^{m}n_j \chi_{S_j}(g) = \sum_{j=1}^{m}\sum_{k=1}^{\ell_j} n_j \chi_{W_{j,k}}(g).
        \end{align}
    \end{itemize}
\end{frame}
...
\begin{center}
\(\highlight[gold]{\text{\Large Haar--Moore chains:}}\)
\vspace{0.2cm}
...
\begin{frame}{Research Questions}{Relations to $B\G$, Haar-measures, étale case.}
\begin{center}
\(\highlight[gold]{\text{\Large Mayer--Vietoris beyond discreteness:}}\)
\vspace{0.2cm}
...
\begin{frame}{Research Questions}{Relations to $B\G$, Haar-measures, étale case.}
\begin{center}
\(\highlight[gold]{\text{\Large Comparison with classifying space homology:}}\)
\vspace{0.2cm}
...
Use
\[
\iota_n^*\colon C({\G}_n,\ZZ)\to C(X,\ZZ), \quad
\pi_n^*\colon C(X,\ZZ)\to C({\G}_n,\ZZ),
\]
\[
\begin{aligned}
\highlight[red]{\bigl((\pi_n)_*(\pi_n)^*f\bigr)(x)} &= \sum_{\gamma\in \pi_n^{-1}(x)} (\pi_n)^*f(\gamma) = \sum_{\gamma\in \pi_n^{-1}(x)} f(\pi_n(\gamma)) = f\bigl(\pi_n(\iota_n(x))\bigr) = \highlight[red]{f(x)},
,\\
\highlight[red]{\bigl((\pi_n)^*(\pi_n)_*g\bigr)(\gamma)} &= (\pi_n)_*g(\pi_n(\gamma)) = \sum_{\eta\in \pi_n^{-1}(\pi_n(\gamma))} g(\eta) = g(\gamma).
\end{aligned}
\]
\end{frame}
...
\[
\begin{aligned}
\bigl((\pi_{n-1})_*(d_i)_*(\pi_n)^*f\bigr)(x)
&=
\sum_{u\in \pi_{n-1}^{-1}(x)} \bigl((d_i)_*(\pi_n)^*f\bigr)(u)\\
&=
\sum_{\gamma\in d_i^{-1}(\iota_{n-1}(x))} \bigl((\pi_n)^*f\bigr)(\gamma)\\
&=
\bigl((\pi_n)^*f\bigr)(\iota_n(x))\\
&=
f(\pi_n(\iota_n(x)))
=
f(x),
\end{aligned}
\]
where we used $\pi_{n-1}^{-1}(x)=\{\iota_{n-1}(x)\}$, $d_i^{-1}(\iota_{n-1}(x))=\{\iota_n(x)\}$, and $\pi_n\iota_n=\mathrm{id}_X$.
Hence
\(
\highlight[red]{(\pi_{n-1})_*(d_i)_*(\pi_n)^*=\mathrm{id}_{C(X,\ZZ)}.}
\)\pause
Using $(\pi_{n-1})^*(\pi_{n-1})_*=\mathrm{id}_{C(\G_{n-1},\ZZ)}$ and $(\pi_n)_*(\pi_n)^*=\mathrm{id}_{C(X,\ZZ)}$ this implies
\[
\highlight[red]{(d_i)_*}=(\pi_{n-1})^*(\pi_{n-1})_*(d_i)_*(\pi_n)^*(\pi_n)_*=(\pi_{n-1})^*\,\mathrm{id}_{C(X,\ZZ)}\,(\pi_n)_*=\highlight[red]{(\pi_{n-1})^*(\pi_n)_*.}
\]
\end{frame}

\begin{frame}{$H_\bullet(\G;\ZZ) \neq H_\bullet(B\G;\ZZ)$}{Unit groupoid of the Cantor set $X$.}
Therefore, under the identifications of $C(\G_n,\ZZ)$ with $C(X,\ZZ)$ via $\pi_n$, one has
\[
(\pi_{n-1})_*\partial_n(\pi_n)^*
=
\sum_{i=0}^n(-1)^i(\pi_{n-1})_*(d_i)_*(\pi_n)^*
=
\begin{cases}
0, & n \text{ odd},\\
\mathrm{id}_{C(X,\ZZ)}, & n \text{ even}.
\end{cases}
\]
Hence the Moore complex is identified with
\[
\cdots \xrightarrow{\ \mathrm{id}\ } C(X,\ZZ) \xrightarrow{\ 0\ } C(X,\ZZ) \xrightarrow{\ \mathrm{id}\ } C(X,\ZZ) \xrightarrow{\ 0\ } C(X,\ZZ) \xrightarrow{\ 0\ } 0.
\]
Consequently
\[
H_0({\G};\ZZ)=C(X,\ZZ),
...
\]
Then $\theta'$ is simplicial and can be extended such that for all $t\in\Delta^m$, $s\in[0,1]$,
\[
\widetilde H((\theta(t),n),s)=\theta'\bigl(\widetilde H((t,m),s)\bigr)
\Rightarrow
\widetilde H((\theta(t),n),s)\approx \widetilde H((t,m),s).
\]
\end{frame}
